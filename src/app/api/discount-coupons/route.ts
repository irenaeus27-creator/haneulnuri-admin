...
export async function GET() {
  try {
    const [discountCoupons, rawSettings] = await Promise.all([
      fetchSheet("discountCoupons"),
      fetchSheet("settings"),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === "add") {
      const result = await postToAppsScript("addDiscountCoupon", data);
    ...
    if (mode === "update") {
      const result = await postToAppsScript("updateDiscountCoupon", data);
    ...
    console.error("[discount-coupons POST error]", error);
